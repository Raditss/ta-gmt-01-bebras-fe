...
import ContagionProtocolSubmitButton from '@/components/features/question/contagion-protocol/solver/submit-button';
import { useDuration } from '@/hooks/useDuration';
import { useAuthStore } from '@/store/auth.store';
import { TimeProgressBar } from '@/components/features/question/shared/time-progress-bar';

export default function ContagionProtocolSolver({
  questionId
}: BaseSolverProps) {
  const { user } = useAuthStore();
  const { question, questionMetadata, loading, error, currentDuration } =
    useSolveQuestion<ContagionProtocolSolveModel>(
      questionId,
  ...
  const monsters = question.content.monsters;
  const answers = question.answer.monsters;
  const currentMonster = monsters.find((monster) => monster.id === currentIdx);
      ...
      <div className="max-w-[95%] mx-auto p-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
          ...
          <div className="h-full">
            <MonsterList
              monstersQuestion={monsters}
              monstersAnswer={answers}
              onClick={handleMonsterListOnClick}
            />

            <ContagionProtocolSubmitButton
              question={question}
