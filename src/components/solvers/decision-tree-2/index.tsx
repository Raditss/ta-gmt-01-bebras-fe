...
import { useDecisionTree2Attempt } from "@/hooks/useDecisionTree2Attempt";
...
import { extractSpriteOptions } from "../decision-tree/helper";
...
import { DecisionTree2 } from "@/components/decision-tree-2/shared/tree";
import { CombinationAnswer } from "@/model/decision-tree-2/question/model";
...
export default function DecisionTree2Solver({ questionId }: BaseSolverProps) {
  ...
  const { question, loading, error, currentDuration } =
    useDecisionTree2Attempt(questionId);
  ...
  const handleSubmit = useCallback(async () => {
    if (!question || !user?.id || !isCorrect) return;
