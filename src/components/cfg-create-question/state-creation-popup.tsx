...
export function StateCreationPopup({ 
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-yellow-400 rounded-lg p-6 w-full max-w-4xl">
        ...
        {mode === 'start' ? (
          <div>
            <div className="bg-white p-6 rounded-md shadow-md mb-6">
              <div className="flex flex-wrap justify-center gap-4">
                {startState.map((obj: { type: string; }, idx: number) => (
            ...
            <div className="text-center mb-4">
              ...
              <div className="flex justify-center gap-4">
                {getAvailableObjectsForStart().map((obj: { type: string; }, idx: number) => (
                  <button 
                    key={idx}
                    onClick={() => addObjectToStartState(obj)}
                    className="w-16 h-16 bg-gray-200 rounded-md flex items-center justify-center hover:bg-gray-300"
                  >
                    {obj.type === 'circle' ? (
                      <div className="w-12 h-12 bg-gray-300 rounded-full"></div>
                    ) : (
          ...
          <div>
            <div className="bg-white p-6 rounded-md shadow-md mb-6">
              <div className="flex flex-wrap justify-center gap-4">
                {endState.map((obj: { type: string; }, idx: number) => (
                  <div 
                    key={idx} 
                ...
                <div className="flex justify-center gap-4">
                  {applicableRules.length > 0 ? (
                    applicableRules.map((rule, idx) => (
                      <button 
                        ...
                        onClick={() => {
                          applyRuleToEndState(selectedObjects, {
                            after: rule.after,
                            id: rule.id // <-- always use the rule's id here
                          });
                          setSelectedObjects([]);
                        }}
                      ...
                      >
                        <div className="flex">
                          {rule.before.map((obj: { type: string; }, idx: number) => (
                            <div key={idx} className="w-8 h-8 mx-1">
                              {obj.type === 'circle' ? (
