...
import { DecisionTree2Question } from "@/model/decision-tree-2/question/model";
import { QuestionType } from "@/constants/questionTypes";

export const useDecisionTree2Attempt = (
  questionId: string,
  shouldSaveAttempt: boolean = true
) => {
  const { user } = useAuth();
  const [question, setQuestion] = useState<DecisionTree2Question | null>(null);
  ...
  const initializeAttemptData = (
    q: DecisionTree2Question,
    duration: number
  ) => {
  ...
  useEffect(() => {
    const fetchQuestionAndAttempt = async () => {
      try {
        ...
        const q = new DecisionTree2Question(
          questionData.id,
          questionData.title,
        ...
        if (user?.id && shouldSaveAttempt) {
          ...
          if (latestAttempt && latestAttempt.status === "paused") {
            ...
            // Restore the attempt state if it's a decision tree 2 question
            if (questionData.type === "decision-tree-2") {
