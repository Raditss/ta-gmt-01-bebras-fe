...
import { DecisionTreeQuestion } from "@/model/decision-tree/question/model";
import { QuestionType } from "@/constants/questionTypes";

export const useDecisionTreeAttempt = (
  questionId: string,
  shouldSaveAttempt: boolean = true
) => {
  const { user } = useAuth();
  const [question, setQuestion] = useState<DecisionTreeQuestion | null>(null);
  ...
  const initializeAttemptData = (q: DecisionTreeQuestion, duration: number) => {
  ...
  useEffect(() => {
    const fetchQuestionAndAttempt = async () => {
      try {
        ...
        const q = new DecisionTreeQuestion(
          questionData.id,
          questionData.title,
        ...
        if (user?.id && shouldSaveAttempt) {
          ...
          if (latestAttempt && latestAttempt.status === "paused") {
            ...
            // Restore the attempt state if it's a decision tree question
            if (questionData.type === "decision-tree") {
